...
export default {
	...
	writing: (generator, extensionConfig) => {
		if (!extensionConfig.languageContent) {
			extensionConfig.languageFileName = validator.sanitizeFilename(
				extensionConfig.languageId + ".tmLanguage.json",
			);

			generator.fs.copyTpl(
				generator.templatePath("syntaxes/new.tmLanguage.json"),
			...
			generator.fs.copyTpl(
				generator.templatePath("syntaxes/language.tmLanguage"),
				generator.destinationPath(
					"syntaxes",
		...
		generator.fs.copyTpl(
			generator.templatePath("vsc-extension-quickstart.md"),
			generator.destinationPath("vsc-extension-quickstart.md"),
			extensionConfig,
		);
		generator.fs.copyTpl(
			generator.templatePath("language-configuration.json"),
...
function askForLanguageExtensions(generator, extensionConfig) {
	...
	return generator
		...
		.then((extAnswer) => {
			extensionConfig.languageExtensions = extAnswer.languageExtensions
				.split(",")
				.map((e) => {
					return e.trim();
				});
...
function convertGrammar(location, extensionConfig) {
	...
	if (location.match(/\w*:\/\//)) {
		...
		return request.xhr({ url: location }).then((r) => {
			if (r.status == 200) {
				let contentDisposition =
					r.headers && r.headers["content-disposition"];
				...
				return Promise.reject(
					"Problems loading language definition file: " +
						r.responseText,
				);
			}
		});
		...
		try {
			...
			return Promise.reject(
				"Problems loading language definition file: " + error.message,
			);
		}
		if (body) {
			return processContent(
			...
			return Promise.reject(
				"Problems loading language definition file: Not found",
			);
		}
	}
}
...
function processContent(extensionConfig, fileName, body) {
	...
	if (path.extname(fileName) === ".json") {
		try {
			...
			return Promise.reject(
				"Language definition file could not be parsed asn JSON: " +
					e.toString(),
			);
		}
	} else {
		if (body.indexOf("<!DOCTYPE plist") === -1) {
			return Promise.reject(
				"Language definition file does not contain 'DOCTYPE plist'. Make sure the file content is really plist-XML.",
			);
		}
		...
		try {
			...
			return Promise.reject(
				"Language definition file could not be parsed: " + e.toString(),
			);
		}
	}
	if (!languageInfo) {
		return Promise.reject(
			"Language definition file could not be parsed. Make sure it is a valid plist or JSON file.",
		);
	}
	...
	if (!languageId && fileName) {
		...
		if (
			lastIndexOfDot2 &&
			fileName.substring(lastIndexOfDot2 + 1) == "tmLanguage"
		) {
	...
	if (Array.isArray(languageInfo.fileTypes)) {
		extensionConfig.languageExtensions = languageInfo.fileTypes.map(
			function (ft) {
				return "." + ft;
			},
		);
	} else {
		extensionConfig.languageExtensions = languageId
			? ["." + languageId]
			: [];
	}
