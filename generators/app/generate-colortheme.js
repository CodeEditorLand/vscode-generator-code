	...
	writing: (generator, extensionConfig) => {
		if (extensionConfig.tmThemeFileName) {
			generator.fs.copyTpl(
				generator.templatePath("themes/theme.tmTheme"),
		...
		extensionConfig.themeFileName = validator.sanitizeFilename(
			extensionConfig.themeName + "-color-theme.json",
		);
		if (extensionConfig.themeContent) {
			extensionConfig.themeContent.name = extensionConfig.themeName;
			generator.fs.copyTpl(
				generator.templatePath("themes/color-theme.json"),
				generator.destinationPath(
					"themes",
		...
		return request.xhr({ url: location }).then((r) => {
			if (r.status == 200) {
				...
				return Promise.reject(
					"Problems loading theme: HTTP status " + r.status,
				);
			}
		});
	} else {
...
function processContent(extensionConfig, tmThemeFileName, body, generator) {
	...
	try {
		extensionConfig.themeContent = migrate(
			body,
			tmThemeFileName,
			generator,
		);
		...
		for (let entry of settings) {
			...
			if (scope) {
				...
				if (notSupported.length > 0) {
					generator.log(
						"Note: the following theming properties are not supported by VS Code and will be ignored: " +
							notSupported.join(", "),
					);
				}
			}
