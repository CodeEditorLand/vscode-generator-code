...
function askForLanguageId(generator, extensionConfig) {
	...
	generator.log(
		"Enter the language identifier as used on transifex (e.g. bg, zh-Hant).",
	);
	return generator
		.prompt({
			type: "input",
		...
		.then((answer) => {
			...
			if (!generator.options["extensionId"]) {
				generator.options["extensionId"] =
					"vscode-language-pack-" + answer.lpLanguageId;
			}
...
function askForLanguageName(generator, extensionConfig) {
	extensionConfig.isCustomization = true;
	generator.log(
		"Enter the language name in English (e.g. 'Bulgarian', 'Dutch').",
	);
	return generator
		.prompt({
			type: "input",
		...
		.then((answer) => {
			...
			if (!generator.options["extensionDisplayName"]) {
				generator.options["extensionDisplayName"] =
					answer.lpLanguageName + " Language Pack";
			}
			if (!generator.options["extensionDescription"]) {
				generator.options["extensionDescription"] =
					"Language pack extension for " + answer.lpLanguageName;
			}
			return Promise.resolve();
		});
}
...
function askForLocalizedLanguageName(generator, extensionConfig) {
	extensionConfig.isCustomization = true;
	generator.log(
		"Enter the language name in " + extensionConfig.lpLanguageName,
	);
	return generator
		.prompt({
			type: "input",
		...
		.then((answer) => {
			extensionConfig.lpLocalizedLanguageName =
				answer.lpLocalizedLanguageName;
			return Promise.resolve();
		});
}
