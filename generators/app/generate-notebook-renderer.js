...
export default {
	...
	writing: (generator, extensionConfig) => {
		...
		generator.fs.copyTpl(
			generator.templatePath("example/notebook.ipynb"),
			generator.destinationPath("example/notebook.ipynb"),
			extensionConfig,
		);
		generator.fs.copyTpl(
			generator.templatePath("src/client/index.ts"),
			generator.destinationPath("src/client/index.ts"),
			extensionConfig,
		);

		generator.fs.copyTpl(
			generator.templatePath("src/extension/extension.ts"),
			generator.destinationPath("src/extension/extension.ts"),
			extensionConfig,
		);
...
async function askForNotebookRendererInfo(generator, extensionConfig) {
	const answers = await generator.prompt([
		...
		},
		{
			...
			message:
				"What mime types will your renderer handle? (separate multiple by commas)",
