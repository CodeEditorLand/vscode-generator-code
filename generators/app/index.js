...
export default class extends Generator {
	...
	async initializing() {
		...
		if (!this.extensionConfig.insiders) {
			this.log(
				yosay("Welcome to the Visual Studio Code Extension generator!"),
			);
		} else {
			this.log(
				yosay(
					"Welcome to the Visual Studio Code Insiders Extension generator!",
				),
			);
		}
	...
	async prompting() {
		...
		if (extensionType) {
			...
			for (const g of extensionGenerators) {
				const name = this.extensionConfig.insiders
					? g.insidersName
					: g.name;
				if (name) {
	...
	writing() {
		...
		if (!this.options["destination"] && !this.extensionGenerator.update) {
			this.destinationRoot(
				this.destinationPath(this.extensionConfig.name),
			);
		}
		...
		this.sourceRoot(
			path.join(
				currentFilename,
				"../templates/" + this.extensionConfig.type,
			),
		);
	...
	async end() {
		...
		if (this.extensionConfig.proposedAPI) {
			await this.spawn(this.extensionConfig.pkgManager, [
				"run",
				"update-proposed-api",
			]);
		}
		...
		const [codeStableLocation, codeInsidersLocation] = await Promise.all([
			...
			which("code-insiders").catch(() => undefined),
		]);
